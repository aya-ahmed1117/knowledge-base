...
<?php 
  $selected = sqlsrv_query($con ,"SELECT *
   FROM [EPMKS].[dbo].[tbl_Projects_details_old5] where project_id =1");
   while($output_query = sqlsrv_fetch_array($selected)){
   $string2 = $output_query["PO_PDF_1"];
$string3 = $output_query["Presales_solution"];
...
<a class="input-group-addon clip" href='.$output_query["PO_PDF_1"].' download>
  <embed src="'.$output_query["PO_PDF_1"].'" style="width:50px;hight:50px;">
</a>
<a class="input-group-addon clip" href='.$output_query["Presales_solution"].' download>
  <embed src="'.$output_query["Presales_solution"].'" style="width:50px;hight:50px;">
</a>';
}
?>
   <!--  <embed src="<?php echo $string2 ; ?>" type="application/pdf">
    <embed src="<?php echo $string3 ; ?>" type="application/pdf"> -->
  ...
  <div class="card-body card-block">
      <form method="post" action="ADD_newOrder.php" enctype="multipart/form-data">
...
<datalist id="browser" name="Customer_name" autocomplete="off" >
...
<div style="clear: both;">
  ...
  <!-- <button type="submit" class="btn btn-default" name="roll" <?php echo isset($_POST["roll"]) ? "disabled" : "";?>>Roll Branch </button>  -->
      ...
      <button type="submit" class="btn-submit submit" id="mySubmit" onclick="myFunction()" name="save" 
      style="width: 30%;padding: 10px;color: #fff;font-size: 15px; border-radius: 20px 20px 20px 20px;background: linear-gradient(to left, #383f88 1%, #713391 60%);" title="Add New Order">Add New
    </button>
</div>
